...
try:
    ...
    # Clean ServiceDateTime — remove seconds (11:22:00 → 11:22)
    if 'ServiceDateTime' in df_easter.columns:
        df_easter['ServiceDateTime'] = df_easter['ServiceDateTime'].astype(str).str.replace(r':00
...
if df_saturated is not None:
    ...
    with filter_col2:
        if 'Campus' in df_easter.columns:
            campus_options = ["All"] + sorted(df_easter['Campus'].dropna().unique().tolist())
    ...
    st.download_button(
        ...
        data=easter_csv,
        file_name=f"Easter_2026_Projections_{day_filter}_{campus_filter}.csv",
        mime="text/csv"
    )
